...
const Auth = () => {
  ...
  const login = useCallback(async (e : any) => {
    ...
    try {
      ...
      setIsLoading(false);
      if (res?.error) {
        setIsError({ value: true, msg: res?.error });
        console.log(res);
      } else {
  ...
  const register = useCallback(async (e : any) => {
    ...
    try {
      ...
      setIsLoading(false);
      console.log(error);
      setIsError({ value: true, msg: error['response']['data']['error'] });
    }
        ...
        <div className="flex justify-center">
          <div className="bg-black bg-opacity-70 px-16 py-16 self-center mt-2 lg:w-2/5 lg:max-w-md rounded-md w-full">
            ...
            <div className="flex flex-col gap-4">
              {variant === 'register' && (
                <Input
                  ...
                  onFocus={() => setIsError({ value: false, msg: '' })}
            ...
            <button onClick={variant === 'login' ? login : register} disabled={isLoading} className="bg-red-600 py-3 text-white rounded-md w-full mt-10 hover:bg-red-700 transition">
              ...
              {isLoading ? <svg aria-hidden="true" role="status" className="inline w-4 h-4 ml-3 text-white animate-spin" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <p className="text-neutral-500 mt-2 mb-2 text-red-600">
              { isError.value ? `${isError.msg}` : '' }
            </p>
